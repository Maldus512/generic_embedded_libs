...
static pid_control_type_t error(pid_control_type_t in, pid_control_type_t sp) {
    return sp - in;
}
...
pid_output_type_t pid_compute(pid_ctrl_t *pid) {
    ...
    pid_control_type_t pterm = pid->Kp * error(pid->input, pid->sp);

    pid_control_type_t iterm = 0;
    for (int i = 0; i < PID_HISTORY_SIZE; i++)
        iterm += pid->history[i];
    iterm *= pid->Ki;

    pid_control_type_t dterm = pid->Kd * (pid->input - pid->history[last]);
...
void pid_clear_history(pid_ctrl_t *pid) {
    pid->index = 0;
    memset(pid->history, 0, sizeof(pid_control_type_t) * PID_HISTORY_SIZE);
}